...
for deviceName,deviceValue in pairs(otherdevices) do
    ...
    if (type == 'PIR' and id == '0') then
        print ('lightintensity: ' .. name .. ': ' .. tostring(otherdevices[name]) .. '. sleeptime: ' .. tostring(sleeptime) .. ' Dim: ' .. tostring(otherdevices['Group:' .. name .. 'Dim']) .. ' Regular: ' .. tostring(otherdevices['Group:' .. name .. 'Regular']))
